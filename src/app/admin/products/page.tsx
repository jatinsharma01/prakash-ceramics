...
import { ADMIN_INVENTORY_ITEMS } from "@/lib/adminData";
...
export default function AdminProductsPage() {
  ...
  const [selectedProduct, setSelectedProduct] = useState<typeof ADMIN_INVENTORY_ITEMS[0] | null>(null);
  const [toastMessage, setToastMessage] = useState<string | null>(null);

  // Filtered Products
  const filteredProducts = useMemo(() => {
    return ADMIN_INVENTORY_ITEMS.filter((item) => {
      const matchesSearch = 
  ...
  const totalInventoryValue = ADMIN_INVENTORY_ITEMS.reduce(
    (acc, curr) => acc + curr.price * curr.stockCount,
    0
  );

  const lowStockCount = ADMIN_INVENTORY_ITEMS.filter((p) => p.stockCount <= 5).length;
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="p-4 rounded-xl bg-[#141822] border border-stone-800 flex items-center justify-between">
          <div>
            ...
            <p className="text-xl font-bold text-white mt-1">{ADMIN_INVENTORY_ITEMS.length} Models</p>
          </div>
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-1 bg-stone-900/90 p-1 rounded-xl border border-stone-800 text-xs">
              ...
              >
                All ({ADMIN_INVENTORY_ITEMS.length})
              </button>
              <button
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl whitespace-nowrap transition-all border ${
              selectedCategory === "all"
                ...
                : "bg-stone-800/60 text-stone-300 border-stone-700 hover:text-white hover:bg-stone-800"
            }`}
          >
            All Collections
          </button>
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl whitespace-nowrap transition-all border ${
                selectedCategory === cat.slug
                  ? "bg-gradient-to-r from-[#9b7842] to-[#836433] text-white font-bold border-[#dec49a]/40 shadow-sm"
                  : "bg-stone-800/60 text-stone-300 border-stone-700 hover:text-white hover:bg-stone-800"
              }`}
            >
              {cat.name}
                  ...
                  <tr key={p.id} className="hover:bg-stone-800/40 transition-colors group">
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        ...
                        <button
                          onClick={() => showToast(`Inventory synced for ${p.name}`)}
